...
Includes full 16-round Feistel structure, S-boxes, permutation tables.
"""
...
def bin2dec(binary):
    binary, decimal, i = int(binary), 0, 0
    while binary != 0:
...
def encrypt(pt, rkb, rk):
    pt = permute(hex2bin(pt), initial_perm, 64)
    ...
    for i in range(16):
        ...
        print(f"Round {i+1:2d}  {bin2hex(left)}  {bin2hex(right)}  {rk[i]}")
    return permute(left + right, final_perm, 64)

if __name__ == "__main__":
    pt  = "02468ACEEACA8642"
    key = "1E030F03080D2930"
    ...
    print("\n=== Decryption ===")
    plain_text = bin2hex(encrypt(cipher_text, rkb[::-1], rk[::-1]))
